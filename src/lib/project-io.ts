...
import type { Brief, ExecutionMode, Scene, SceneRole, ShortsProject } from "../types";
...
export function parseBriefPayload(raw: unknown): ImportedBriefPayload {
  ...
  const brief: Brief = {
    ...
    tone: unionField(record.tone, ["neutral", "serious", "energetic", "urgent"], "serious"),
    targetDuration: numberField(record.targetDuration, 30),
    platform: unionField(record.platform, ["youtube", "tiktok", "reels"], "youtube"),
    language: unionField(record.language, ["ko", "en"], "ko"),
    audience: stringField(record.audience, "의사결정 구조를 빠르게 공유하고 싶은 실무자"),
    thesis: stringField(record.thesis, "좋은 숏츠는 구조를 먼저 세우는 데서 시작됩니다."),
  };

  const script = stringField(
    record.script,
    [brief.title, brief.thesis, "핵심 메시지를 문장 단위로 더 적어주세요."].join("\n"),
  );
...
export function hydrateProject(raw: unknown): ShortsProject {
  ...
  if ("brief" in record && "script" in record) {
    ...
    const base = createProjectFromBrief(briefPayload.brief, stringField(record.script, briefPayload.script), {
    ...
    return recalculateProject({
      ...base,
      scenes,
      status: unionField(record.status, ["editing", "review", "rendered", "failed"], base.status),
    });
  }
...
function hydrateScene(raw: unknown, index: number, brief: Brief): Scene | null {
  try {
    const record = asRecord(raw);
    const role = unionField(record.role, ["hook", "build", "payoff", "cta"], index === 0 ? "hook" : "build");

    return refreshSceneFromText(
      {
        id: stringField(record.id, `scene-${index + 1}`),
        index: numberField(record.index, index + 1),
        text: stringField(record.text, brief.thesis),
        duration: numberField(record.duration, Math.max(2.8, brief.targetDuration / 7)),
        role,
        media: {
          type: unionField(asRecord(record.media).type, ["image", "gif", "video"], "image"),
          query: stringField(asRecord(record.media).query, brief.topic),
          style: stringField(asRecord(record.media).style, "editorial still"),
          tags: arrayOfStrings(asRecord(record.media).tags),
          sourceHint: stringField(asRecord(record.media).sourceHint, "editorial still"),
        },
        subtitles: {
          id: stringField(asRecord(record.subtitles).id, `subtitle-${index + 1}`),
          lines: arrayOfStrings(asRecord(record.subtitles).lines).slice(0, 2),
          emphasis: arrayOfStrings(asRecord(record.subtitles).emphasis).slice(0, 2),
        },
        voice: {
          provider: stringField(asRecord(record.voice).provider, "TTS placeholder"),
          speed: numberField(asRecord(record.voice).speed, 1),
          emotion: unionField(asRecord(record.voice).emotion, ["neutral", "serious", "energetic"], "neutral"),
        },
...
function hydrateRuntime(raw: Record<string, unknown>, fallbackMode: ExecutionMode) {
  const defaults = buildRuntimeProfile(fallbackMode);

  return {
    mode: unionField(raw.mode, ["local", "byo-api", "hybrid"], defaults.mode),
    costModel: unionField(raw.costModel, ["free-local", "user-api", "mixed"], defaults.costModel),
    ...
    install: arrayOfStrings(raw.install).length ? arrayOfStrings(raw.install) : defaults.install,
  };
}
...
function unionField<T extends string>(value: unknown, allowed: readonly T[], fallback: T): T {
