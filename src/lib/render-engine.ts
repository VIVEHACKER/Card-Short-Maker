...
export async function renderProjectToMp4(options: RenderProjectOptions): Promise<{
  outputFile: string;
  workspaceDir: string;
}> {
  ...
  await fs.writeFile(path.join(workspaceDir, "subtitles.srt"), buildSrt(options.project), "utf8");
  ...
  for (const scene of options.project.scenes) {
    ...
    const textFiles = await writeSceneTextFiles(sceneDir, options.project, scene);
    ...
    if (options.withTts !== false) {
      ...
      const { voice } = await synthesizeSceneAudio({
        ...
        preferredVoice: options.project.brief.language === "ko" ? "Microsoft Heami Desktop" : undefined,
    ...
    const segmentPath = path.join(segmentsDir, `${scene.index.toString().padStart(2, "0")}-${scene.id}.mp4`);
  ...
  const outputFile = path.resolve(options.outputFile ?? path.join(outputDir, `${slugify(options.project.brief.title)}.mp4`));
  await execFileAsync(
    ffmpeg,
    [
      "-y",
...
async function writeSceneTextFiles(baseDir: string, project: ShortsProject, scene: Scene) {
  ...
  await fs.writeFile(sceneFile, wrapText(scene.text, project.brief.language === "ko" ? 12 : 18).join("\n"), "utf8");
  await fs.writeFile(roleFile, `${scene.index}. ${scene.role.toUpperCase()}`, "utf8");
...
async function resolveFfmpegPath(explicitPath?: string) {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(command, [target], { windowsHide: true });
