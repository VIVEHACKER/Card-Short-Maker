...
function buildPackageReadme(project: ShortsProject): string {
  ...
  return [
    ...
    `Template: ${manifest.template.label}`,
    `Layout: ${manifest.template.layout}`,
    `Fixed Format: ${manifest.template.fixedFormat ? "yes" : "no"}`,
    ...
    "- subtitles.srt: subtitle timeline (FFmpeg / standard)",
    "- subtitles.vtt: WebVTT for HTML5 / web players",
    "- subtitles.ass: ASS / SubStation Alpha for advanced styling",
