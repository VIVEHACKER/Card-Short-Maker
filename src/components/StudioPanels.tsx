  ...
  return (
    <div className="panel-scroll">
      <div className="overview-grid">
        ...
        <MetricCard label="플랫폼" value={`${project.brief.platform} · ${project.brief.intent}`} />
        <MetricCard label="러닝타임" value={`${project.brief.targetDuration}초`} />
      </div>
...
export function EditorCanvas({
  ...
  return (
    <div className="preview-stack">
      ...
      <Timeline activeSceneId={scene.id} scenes={project.scenes} onSelectScene={onSelectScene} />
    </div>
  );
}

export function ReviewCanvas({ project, scene }: { project: ShortsProject; scene: Scene }) {
  return (
    <div className="panel-scroll">
...
export function ExportCanvas({ project, renderPackage }: { project: ShortsProject; renderPackage: RenderPackage }) {
  return (
    <div className="panel-scroll">
      ...
      <div className="detail-grid">
        <DetailCard icon={<BadgeCheck size={15} />} label="실행 모드" value={project.runtime.mode} />
        <DetailCard icon={<BadgeCheck size={15} />} label="비용 모델" value={project.runtime.costModel} />
        <DetailCard icon={<BadgeCheck size={15} />} label="스크립트 / QA" value={`${project.runtime.scriptProvider} / ${project.runtime.qaProvider}`} />
        <DetailCard icon={<BadgeCheck size={15} />} label="TTS / Media" value={`${project.runtime.ttsProvider} / ${project.runtime.mediaProvider}`} />
      </div>

      <div className="file-list">
        <FileRow name="project.json" description="편집 상태 전체를 담는 프로젝트 파일" />
        <FileRow name="render-manifest.json" description="렌더 엔진이 읽는 장면 매니페스트" />
      ...
      <div className="command-box">
        ...
        <pre>{`npm run cli -- package --project ./project.json --out ./render-pack`}</pre>
      </div>
...
export function PhonePreview({ project, scene }: { project: ShortsProject; scene: Scene }) {
  return (
      ...
      >
        ...
        <div className="player-bar">
          <div className="player-bar__track">
            <span style={{ width: `${(scene.index / project.scenes.length) * 100}%` }} />
          </div>
          <div className="player-bar__meta">
            <span>
              {secondsFromScenes(project.scenes, scene.index - 1).toFixed(2)} / {project.brief.targetDuration.toFixed(0)}초
            </span>
  ...
  return (
    <div className="score-row">
      ...
      <div className="score-row__track">
        <motion.span initial={{ width: 0 }} animate={{ width: `${value}%` }} transition={{ duration: 0.4, ease: "easeOut" }} />
      </div>
    </div>
  );
}
...
export function QuantChip({ label, status }: { label: string; status: "ok" | "warn" }) {
  return (
